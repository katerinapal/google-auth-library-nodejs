...
exports.OAuth2Client = undefined;
 ...
 */
_util2.default.inherits(OAuth2Client, _authclient.AuthClient);
...
OAuth2Client.prototype.generateAuthUrl = function (opt_opts) {
  ...
  return rootUrl + '?' + _querystring2.default.stringify(opts);
};
...
OAuth2Client.prototype.getToken = function (code, opt_callback) {
  ...
  this.transporter.request({
    ...
    var done = opt_callback || _lodash2.default;
...
OAuth2Client.prototype.revokeToken = function (token, opt_callback) {
  this.transporter.request({
    uri: OAuth2Client.GOOGLE_OAUTH2_REVOKE_URL_ + '?' + _querystring2.default.stringify({ token: token }),
    json: true
  }, opt_callback);
};
...
OAuth2Client.prototype.request = function (opts, callback) {
  ...
  var postRequestCb = function postRequestCb(err, body, resp) {
    ...
    if (retry && !err && resp && (resp.statusCode === 401 || resp.statusCode === 403)) {
...
OAuth2Client.prototype.verifySignedJwtWithCerts = function (jwt, certs, requiredAudience, issuers, maxExpiry) {
  ...
  var pemVerifier = new _pemverifier.PemVerifier();
  ...
  return new _loginticket.LoginTicket(envelope, payload);
};
 ...
 * @constructor
 */
exports.OAuth2Client = mod_OAuth2Client;