...
import ext_gToken from "gtoken";
import { JWTAccess as jwtaccess_JWTAccess } from "./jwtaccess.js";
import ext_noop from "lodash.noop";
...
JWT.prototype.getRequestMetadata = function(opt_uri, metadataCb) {
  if (this.createScopedRequired() && opt_uri) {
    ...
    var alt = new jwtaccess_JWTAccess(this.email, this.key);
...
JWT.prototype.authorize = function(opt_callback) {
  var that = this;
  var done = opt_callback || ext_noop;
    ...
    if (err) {
      ...
      return gToken.getToken(function (err, token) {
        return done(err, {
          access_token: token,
          token_type: 'Bearer',
          expiry_date: ext_gToken.expires_at
        });
      });
    }
  });
