var mod_GoogleAuth = GoogleAuth;
import { JWT as JWTClient } from "./jwtclient.js";
import { Compute as ComputeClient } from "./computeclient.js";
...
import { IAMAuth as iamjs } from "./iam.js";
import { JWTAccess as jwtaccessjs } from "./jwtaccess.js";
import { OAuth2Client as oauth2clientjs } from "./oauth2client.js";
import { UserRefreshClient as refreshclientjs } from "./refreshclient.js";
/**
...
GoogleAuth.prototype._checkIsGCE = function(callback) {
  ...
  if (that._checked_isGCE) {
    ...
    if (!that.transporter) {
      that.transporter = new transporters_DefaultTransporter();
    }
  ...
  function(filePath, opt_callback) {
    ...
    if (!error) {
      try {
        ...
        filePath = ext_fs_fs.realpathSync(filePath);

        if (!ext_fs_fs.lstatSync(filePath).isFile()) {
          throw '';
        }
      } catch (err) {
        error = createError(ext_util_util.format('The file at %s does not exist, or it is not a file.',
          filePath), err);
      }
    }
    ...
    if (!error) {
      try {
        ...
        error = createError(ext_util_util.format('Unable to read the file at %s.', filePath), err);
      }
    }
    if (error) {
...
GoogleAuth.prototype._createReadStream = function(filePath) {
  return ext_fs_fs.createReadStream(filePath);
};
...
GoogleAuth.prototype._pathJoin = function(item1, item2) {
  return ext_path_path.join(item1, item2);
};
 ...
 * @constructor
 */
export { mod_GoogleAuth as GoogleAuth };
