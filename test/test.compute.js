...
describe('Compute auth client', function () {
  ...
  it('should get an access token for the first request', function (done) {
    var scope = nock('http://metadata.google.internal').get('/computeMetadata/v1beta1/instance/service-accounts/default/token').reply(200, { access_token: 'abc123', expires_in: 10000 });
  ...
  describe('._injectErrorMessage', function () {
    it('should return a helpful message on request response.statusCode 403', function (done) {
      ...
      compute.request({}, function (err, result, response) {
        ...
        assert.equal('A Forbidden error was returned while attempting to retrieve an access ' + 'token for the Compute Engine built-in service account. This may be because the ' + 'Compute Engine instance does not have the correct permission scopes specified.', err.message);
        done();
      });
    });
    ...
    it('should return a helpful message on request response.statusCode 404', function (done) {
      ...
      compute.request({}, function (err, result, response) {
        ...
        assert.equal('A Not Found error was returned while attempting to retrieve an access' + 'token for the Compute Engine built-in service account. This may be because the ' + 'Compute Engine instance does not have any permission scopes specified.', err.message);
        done();
      });
    });

    it('should return a helpful message on token refresh response.statusCode 403', function (done) {
      nock('http://metadata.google.internal').get('/computeMetadata/v1beta1/instance/service-accounts/default/token').reply(403, 'a weird response body');
      ...
      compute.request({}, function (err, result, response) {
        assert.equal(403, response.statusCode);
        assert.equal('A Forbidden error was returned while attempting to retrieve an access ' + 'token for the Compute Engine built-in service account. This may be because the ' + 'Compute Engine instance does not have the correct permission scopes specified. ' + 'Could not refresh access token.', err.message);
        nock.cleanAll();
        done();
      });
    });
    ...
    it('should return a helpful message on token refresh response.statusCode 404', function (done) {
      nock('http://metadata.google.internal').get('/computeMetadata/v1beta1/instance/service-accounts/default/token').reply(404, 'a weird body');
      ...
      compute.request({}, function (err, result, response) {
        assert.equal(404, response.statusCode);
        assert.equal('A Not Found error was returned while attempting to retrieve an access' + 'token for the Compute Engine built-in service account. This may be because the ' + 'Compute Engine instance does not have any permission scopes specified. Could not ' + 'refresh access token.', err.message);
        done();
      });
    });
  });
